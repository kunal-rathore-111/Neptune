    ...
    return (
      <TreeContext.Provider
        ...
        <div className={cn('size-full', className)}>
          <div ref={ref} className="relative h-full overflow-auto px-2" dir={dir as Direction}>
...
const TreeIndicator = forwardRef<HTMLDivElement, Omit<HTMLMotionProps<'div'>, 'ref'>>(
  ({ className, ...props }, ref) => {
...
const Folder = forwardRef<HTMLDivElement, FolderProps & React.HTMLAttributes<HTMLDivElement>>(
  ({ className, element, value, isSelectable = true, isSelect, children, ...props }, ref) => {
    const { direction, handleExpand, expandedItems, indicator, selectedId, selectItem, openIcon, closeIcon } =
      useTree();
...
>(({ className, elements, expandAll = false, children, ...props }, ref) => {
  ...
  return (
    <Button
      ...
      onClick={expandedItems && expandedItems.length > 0 ? closeAll : () => setExpandedItems?.(expendAllTree(elements))}
      ref={ref}
      {...props}>
      {children}
